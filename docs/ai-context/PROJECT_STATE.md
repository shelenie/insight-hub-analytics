...
The Data Bindings manual/admin UX copy from PR #190 now uses the existing app i18n system instead of hardcoded Ukrainian for the updated tab labels, section descriptions, KPI helper text, and empty states. English translations were added without backend logic, RPC signature, RLS, query, drawer, filter, or table behavior changes.
