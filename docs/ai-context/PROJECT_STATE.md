## 2026-07-19 — Portfolio/demo Imports and Data Bindings polish

The client declined the paid project, so the system is now being maintained with limited polish as a portfolio/demo analytics asset. Imports and Data Bindings received frontend-only responsive and presentation polish: the upload form now fits laptop content widths, imported-source status uses a Select, source rows prioritize human-friendly file metadata, technical storage paths are collapsed, actions align consistently, and the panel is bilingual. No backend logic, route, upload payload, Edge Function contract, Supabase schema, RLS, SQL, or storage policy changed.
...
## 2026-07-16 — PR #269 import cleanup blocker fixes

PR #269 cleanup blockers were addressed locally: `import-source-cleanup` now resolves `import_run_id` before destructive cleanup, uses table-specific cleanup keys for dataset mappings, raw rows, rejected rows, staging rows, mapping review queue, source bindings, datasets, and file assets, and no longer uses a generic `raw_external_dataset_id` delete path for every table. A registry migration now registers `import-source-cleanup` as active, dangerous, audited, admin-access Edge Function for the Analytics Hub workspace while destructive cleanup remains superadmin-only inside the function. Data Bindings cleanup no longer sends active-bound cleanup confirmation automatically; active-bound imports require the separate `cleanup-active` confirmation path. Focused tests now cover table-specific cleanup columns, import-run resolution, registry registration, Storage API deletion, and active-bound UI protection.

## 2026-07-15 — Production source management and import cleanup actions

Imports upload UI is production-accepted and manual DB cleanup was verified, but Storage cleanup is now handled by a production-safe admin/superadmin path instead of manual Supabase Storage deletion. Added `import-source-cleanup` Edge Function for uploaded/imported file sources with admin archive/restore, superadmin confirmed cleanup, service-role Storage API deletion from `file-imports`, workspace scoping, file-import source validation, active-binding confirmation, dependency-order DB cleanup, and audit events for archive/restore/cleanup/failures. Data Bindings → Джерела даних now includes imported-source Active/Archived/All management, Ukrainian archive/cleanup confirmations, and superadmin-only destructive cleanup visibility. Archived uploaded datasets are excluded from active source candidates while Google Sheets and Ads connector behavior remain unchanged.

## 2026-07-15 — Imports upload compact desktop grid follow-up

Frontend-only Imports upload spacing fix: replaced the stretching desktop upload grid column with a left-aligned content-width grid using fixed safe desktop columns (file/header/all-sheets/source/upload), widened the upload button column to 270px, and preserved the selected-file pill/helper placement, global overflow protection, success panel, Data Bindings source-tab CTA, and upload/parser contract. No backend, Supabase schema, migrations, RLS, RPC definitions, Edge Function code, storage bucket, file-upload-parser contract, upload parser payload, upload behavior, Bindings tab query-param behavior, production data, or uploaded files changed.
...
## 2026-07-15 — Imports overflow follow-up for layout wrappers

Imports overflow follow-up: DashboardLayout now adds local max-width/min-width and overflow clipping protection on the sidebar provider, dashboard shell, content column, header, page header, and main content wrappers; the Imports upload form keeps aligned internal label/control/helper rows but now uses a wrapping flex layout instead of the fixed five-column desktop grid. The compact success summary keeps truncation and a safely wrapping right-side CTA/dismiss area. No backend, Supabase schema, migrations, RLS, RPC definitions, Edge Function code, storage bucket, file-upload-parser contract, upload parser payload, OAuth logic, Ads connector logic, Google Sheets connector logic, binding contracts, routes, production data, uploaded files, or Google Drive uploads changed.
...
## Local Frontend Fix — 2026-07-14 Bindings Table Visual Polish

A frontend-only `/bindings` table polish follow-up is prepared locally. Source and Ad Account binding tables now use explicit compact first-column pixel widths (280px Source, 260px Account), rebalanced fixed pixel widths for Client/Project/Funnel/Platform/status/Updated/Action columns, and vertical-middle body/action cells so wrapped primary names no longer make the rest of the row appear top-aligned. Shared `BindingRowActions` now includes `justify-center` while preserving full-width compact buttons and the permission-loading placeholder. Friendly source names, two-line Updated timestamps, filters, archive/restore callbacks, unbound ad-account cards, data queries, permissions, Supabase migrations/RPCs/RLS/Edge Functions, and production data were not changed. Local repository has no Git remote configured, so syncing from latest `main` could not be performed in this container.


## Local Frontend Fix — 2026-07-14 Bindings Real-Page Geometry Review Fix

A frontend-only PR #253 follow-up is prepared locally for the `/bindings` table geometry review. Source and Ad Account binding tables still avoid percentage `<colgroup>` widths, but now use compact real-page minimum widths of 990px and 1030px with fixed pixel compact columns and a flexible Source/Account column intended to fit a 1440px desktop viewport inside the expanded DashboardLayout sidebar. Shared binding action controls use the available compact Action cell width with `items-stretch`, `w-full`, `max-w-full`, and `h-8`, so Rebind, Archive, Restore, and the permission-loading placeholder align consistently without promising an impossible 144px rendered width. The dead standalone browser harness and `test:browser` script were removed; final 1280px/1440px/1700px geometry validation is a manual preview/production check. Existing friendly source names, two-line Updated timestamps, permission-loading behavior, archive/restore callbacks, filters, RPCs, migrations, Edge Functions, unbound ad-account cards, and production data were not changed. PR #253 description has been updated to reflect the 990px/1030px final layout and removal of browser harness claims; real visual verification remains blocked in this container because no browser/runtime is available.


## Local Frontend Fix — 2026-07-13 Bindings Desktop Layout Follow-up

A narrow frontend-only `/bindings` layout follow-up is prepared locally. Source and Ad Account binding tables now use the confirmed desktop column proportions with 13% Updated and 20% Action columns, keep desktop overflow as `overflow-x-auto` instead of hidden clipping, render binding actions through a shared vertical full-width `BindingRowActions` component, and display Updated timestamps as stable two-line date/time values with the full formatted timestamp in `title`. Source archive and restore dialogs now use `formatBindingSourceName(row)` so Google Sheet tabs show the friendly `Sheet · Tab` label instead of the raw `google_sheet:` prefix. No migrations, RPCs, RLS, Edge Functions, permission logic, archive/restore behavior, production data, or unbound ad-account card flow changed. Browser viewport verification at 1440px/1700px was not performed in this environment.
...
## Verified Local Post-QA Fix — 2026-07-13 Source Candidate Privileges and Onboarding Toast Styling

A narrow post-deployment QA fix is prepared locally for the `/bindings` source-candidate list and Onboarding toast styling. A new Supabase migration grants `authenticated` only direct `SELECT` on `google_sheet_sources`, `google_sheet_tabs`, and `raw_external_datasets`, revokes unnecessary direct `MAINTAIN` / `REFERENCES` / `TRIGGER` / `TRUNCATE` privileges, and does not change data, RLS policies, RPC signatures, anon access, Edge Functions, binding mutation behavior, or onboarding create/edit behavior. The Bindings UI now treats source-candidate query failures as temporary-unavailable with retry/refresh and disables source submission without also showing the genuine empty-state message; successful zero-result queries still show the empty source message. Toast visual states are now centralized in the shared toast component with semantic success/error/warning/info variants; Onboarding and Bindings use semantic variants instead of page-local color classes, with Onboarding success toasts using `success` and failures using `error`. Production migration application and live privilege verification still require Supabase migration access.

## PR #245 Final Local Status — 2026-07-12 Step 3 Data Bindings and Onboarding Frontend Actions

Step 3 Data Bindings frontend actions are complete locally for PR #245: `/bindings` uses authenticated hardened RPC helpers for ad-account create/edit/rebind/archive, safe source selectors with two-phase selected source rebind, exact selected archive, explicit primary intent, and member read-only behavior. Onboarding Client/Project/Funnel create uses hardened authenticated `upsert_*` RPCs, while edit mode updates the exact selected row by ID/workspace, supports inactive/archived reactivation, writes server-derived audit metadata, and blocks normal Project/Funnel reparenting. Google Sheet source/tab validation rejects inactive or missing parent Sheets. No migration, RLS change, RPC signature change, production data mutation, or `ai-helper-run` change was made. Five Edge Functions require deployment after merge: `binding-create-or-update`, `binding-archive`, `onboarding-client-upsert`, `onboarding-project-upsert`, and `onboarding-funnel-upsert`; manual production QA is required after deployment.
