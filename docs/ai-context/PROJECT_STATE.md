## Verified Local Follow-up — 2026-07-12 Step 3 Onboarding Reactivation Audit and Reparent Guard Fixes

Step 3 onboarding edit wrappers now allow an authorized admin/superadmin to update or reactivate the exact selected inactive/archived Client, Project, or Funnel row, while still requiring active parent hierarchy for Project and Funnel edits. Edit paths preserve existing metadata, merge only plain-object request metadata, and override actor audit fields with server-derived `authData.user` values (`updated_by`, `updated_by_email`, `updated_via`, `updated_at`). Normal edit mode now blocks Project→Client and Funnel→Project reparenting with dedicated friendly codes and disables hierarchy selectors in the Onboarding UI while editing existing Project/Funnel records. No migration, RLS change, RPC signature change, production data mutation, or `ai-helper-run` change was made.

## Verified Local Follow-up — 2026-07-12 Step 3 Onboarding Edit and Source Parent Validation Fixes

Step 3 follow-up now keeps onboarding creates on the hardened `upsert_client`, `upsert_project`, and `upsert_funnel` RPC paths only when no entity ID is supplied, while edit mode updates the exact selected Client/Project/Funnel row through the authenticated user client scoped by both entity ID and workspace. Project edits validate the selected active Client; Funnel edits validate the selected active Project and derive `client_id` from that Project. Google Sheet tab source candidates and server-side tab resolution now reject active tabs when their parent Google Sheet source is missing or inactive/archived/disabled/deleted. No migration, RLS change, RPC signature change, production data mutation, or `ai-helper-run` change was made.

## Verified Local Follow-up — 2026-07-12 Step 3 RPC Payload and Primary Preservation Fixes

Step 3 follow-up fixes align onboarding frontend helpers and legacy onboarding Edge Function wrappers to the exact hardened `upsert_client`, `upsert_project`, and `upsert_funnel` production argument names, removing unsupported stale parameters while keeping `p_created_by` / `p_created_by_email` null for backend-derived actor identity. Selected source and ad-account rebinds now preserve the original `is_primary` state when the operator leaves primary intent unchanged, while same-scope edits still send null to preserve idempotent primary state. Google Sheet source candidate loading and server-side source resolution now validate `is_active === false` in addition to inactive/archived statuses, and partial source rebind Technical details retain only safe old/new binding IDs. No migration, RLS change, RPC signature change, production data mutation, or `ai-helper-run` change was made.
