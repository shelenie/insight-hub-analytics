...
AdsConnectors → Ad accounts now keeps multi-account readiness visible as a compact account-first summary with friendly overall status, nested summary counters, and a short next action. Detailed platform readiness and binding gaps remain available in an expandable details section, so active bound account cards appear immediately after the summary instead of below large diagnostic tables. Status labels for key readiness and binding-gap codes are localized in Ukrainian and English. Bindings remains the place for fixing binding gaps. No routes, top-level tabs, sidebar items, Supabase schema, RLS, RPC contracts, OAuth, sync behavior, Edge Functions, ai-helper-run, Bindings page, or Imports page changed.
