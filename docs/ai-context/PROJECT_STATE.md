## Verified Local Change — 2026-07-07 AdsConnectors Ad Accounts Readiness UX Polish

AdsConnectors → Ad accounts now keeps multi-account readiness visible as a compact account-first summary with friendly overall status, nested summary counters, and a short next action. Detailed platform readiness and binding gaps remain available in an expandable details section, so Real accounts cards appear immediately after the summary instead of below large diagnostic tables. Status labels for key readiness and binding-gap codes are localized in Ukrainian and English. “Real accounts” remains the label for accounts that truly exist on Meta Ads, Google Ads, or TikTok Ads; those accounts can still be test/non-spending accounts created for site development. Bindings remains the place for fixing binding gaps. No routes, top-level tabs, sidebar items, Supabase schema, RLS, RPC contracts, OAuth, sync behavior, Edge Functions, ai-helper-run, Bindings page, or Imports page changed.
