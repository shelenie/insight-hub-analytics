## Local Frontend Fix — 2026-07-13 Bindings Final Table Geometry

A final frontend-only `/bindings` table geometry correction is prepared locally. Source and Ad Account binding tables no longer use percentage `<colgroup>` widths; compact columns use fixed pixel widths, the Source/Account column remains flexible, and the tables keep full-width fixed layouts with 1180px/1200px minimum widths. Shared binding action buttons no longer stretch to the full Action column and now use consistent 144px by 32px sizing for Rebind, Archive, Restore, and loading states. Existing friendly source names, two-line Updated timestamps, permission-loading behavior, archive/restore callbacks, filters, RPCs, migrations, Edge Functions, unbound ad-account cards, and production data were not changed. A browser-geometry harness was added for 1280px, 1440px, and 1700px viewport checks; local execution requires the Playwright runtime, which is unavailable in this container because package registry access is blocked.
