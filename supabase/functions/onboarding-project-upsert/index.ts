...
Deno.serve(async (req) => {
  ...
  if (!workspace_id || !client_id || !name) return json({ ok: false, error: "workspace_id, client_id, and name are required" }, 400);
  ...
  const { data, error } = await adminClient.rpc("upsert_project", {
    p_workspace_id: workspace_id, p_project_id: body.project_id ?? null, p_client_id: client_id, p_name: name, p_code: body.code ?? null, p_status: body.status ?? null, p_metadata: body.metadata ?? null,
  });
  if (error) return json({ ok: false, error: error.message }, 400);
  return json({ ok: true, project: data, project_id: (data as { id?: string } | null)?.id ?? body.project_id ?? null });
});
