...
const FUNCTION_NAME = "google-ads-oauth-start";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await writeAuditLog({
      ...
      workspaceId,
      actor: null,
      action: "google_ads_oauth_start_permission_check_failed",
      severity: "error",
      metadata: {
  ...
  if (!actor.allowed) {
    await writeAuditLog({
      ...
      workspaceId,
      actor,
      action: "google_ads_oauth_start_denied",
      severity: "warning",
      metadata: {
        reason: actor.reason,
  ...
  let stateToken = "backend_test_state_not_for_real_google_ads_oauth";

  await writeAuditLog({
    supabaseAdmin,
    workspaceId,
    actor,
    action: "google_ads_oauth_start_started",
    severity: "info",
    metadata: {
  ...
  if (actor.mode !== "backend_test") {
    const { data, error } = await supabaseAdmin.rpc("create_ad_oauth_state", {
      ...
      p_redirect_uri: googleAdsRedirectUri,
      p_scopes: scopes,
    ...
    if (error || !data) {
      await writeAuditLog({
        ...
        workspaceId,
        actor,
        action: "google_ads_oauth_start_failed",
        severity: "error",
        metadata: {
      ...
      return jsonResponse(500, {
        ok: false,
        error: "Could not create Google Ads OAuth state.",
  ...
  authorizationUrl.searchParams.set("redirect_uri", googleAdsRedirectUri);
  ...
  await writeAuditLog({
    ...
    action:
      actor.mode === "backend_test"
        ? "google_ads_oauth_start_backend_test_success"
        : "google_ads_oauth_start_success",
    severity: "info",
    metadata: {
      backend_test_only: actor.mode === "backend_test",
  ...
  return jsonResponse(200, {
    ...
    note:
      actor.mode === "backend_test"
        ...
        : "Open this URL to connect Google Ads.",
  });
});
