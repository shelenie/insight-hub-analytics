...
const PROMPT_GUIDANCE_COMPATIBILITY_NOTES = [
  ...
  "Client communication triggers: сформулюй клієнту, напиши клієнту, дай текст клієнту, що сказати клієнту, поясни клієнту, для клієнта, як сформулювати, message for the client, client update, client-ready text, client-ready, send to client, message to client",
...
function buildSystemPrompt(params: {
  ...
  return [
    ...
    "When explicit client communication is requested, wrap only copy-ready client text in [CLIENT_COPY_START] and [CLIENT_COPY_END]. Use those markers only for explicit client communication requests such as сформулюй клієнту, напиши клієнту, дай текст клієнту, що сказати клієнту, для клієнта, message for the client, client update, client-ready text, client-ready, send to client, message to client.",
    "Inside [CLIENT_COPY_START] / [CLIENT_COPY_END], include only text that can be copied and sent to the client. Do not include Можна сформулювати клієнту так:, Ось так:, markdown separator ---, or internal checklist/notes inside the markers. For client-wording-only prompts, return only the marker block with no explanation before/after and no Внутрішньо: що перевірити section. Add internal notes after [CLIENT_COPY_END] only when the user explicitly also asks for an internal checklist, team checks, or internal steps. Keep client text practical, transparent, not blamey, and do not overpromise fixes.",
