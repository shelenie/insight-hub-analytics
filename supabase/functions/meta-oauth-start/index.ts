...
const FUNCTION_NAME = "meta-oauth-start";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await writeAuditLog({
      ...
      workspaceId,
      actor: null,
      action: "meta_oauth_start_permission_check_failed",
      severity: "error",
      metadata: {
  ...
  if (!actor.allowed) {
    await writeAuditLog({
      ...
      workspaceId,
      actor,
      action: "meta_oauth_start_denied",
      severity: "warning",
      metadata: {
        reason: actor.reason,
  ...
  if (actor.mode !== "backend_test") {
    ...
    if (error || !data) {
      await writeAuditLog({
        ...
        workspaceId,
        actor,
        action: "meta_oauth_start_failed",
        severity: "error",
        metadata: {
      ...
      return jsonResponse(500, {
        ok: false,
        error: "Could not create Meta OAuth state.",
  ...
  const authorizationUrl = new URL(
    `https://www.facebook.com/${metaApiVersion}/dialog/oauth`,
  );

  authorizationUrl.searchParams.set("client_id", metaAppId);
  authorizationUrl.searchParams.set("redirect_uri", metaRedirectUri);
  ...
  await writeAuditLog({
    ...
    action:
      actor.mode === "backend_test"
        ? "meta_oauth_start_backend_test_success"
        : "meta_oauth_start_success",
    severity: "info",
    metadata: {
  ...
  return jsonResponse(200, {
    ...
    note:
      actor.mode === "backend_test"
        ...
        : "Open this URL to connect Meta Ads.",
  });
});
