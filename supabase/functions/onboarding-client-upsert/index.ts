...
Deno.serve(async (req) => {
  ...
  if (!workspace_id || !name) return json({ ok: false, error: "workspace_id and name are required" }, 400);
  ...
  const { data, error } = await adminClient.rpc("upsert_client", {
    p_workspace_id: workspace_id,
  ...
  return json({ ok: true, client: data, client_id: (data as { id?: string } | null)?.id ?? body.client_id ?? null });
});
