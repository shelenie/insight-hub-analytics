...
create or replace view public.v_unified_ads_performance_daily
...
select
  ...
  atr.metric_date,
  atr.campaign_name,
  sum(coalesce(atr.spend, 0))::numeric as spend,
  sum(coalesce(atr.clicks, 0))::numeric as clicks,
  sum(coalesce(atr.leads, 0))::numeric as leads,
  sum(coalesce(atr.reach, 0))::numeric as reach,
  case
    when sum(coalesce(atr.clicks, 0)) = 0 then 0::numeric
    else sum(coalesce(atr.spend, 0))::numeric / nullif(sum(coalesce(atr.clicks, 0))::numeric, 0)
  end as cpc,
  case
    when sum(coalesce(atr.leads, 0)) = 0 then 0::numeric
    else sum(coalesce(atr.spend, 0))::numeric / nullif(sum(coalesce(atr.leads, 0))::numeric, 0)
  end as cpl,
  case
    when sum(coalesce(atr.clicks, 0)) = 0 then 0::numeric
    else (sum(coalesce(atr.leads, 0))::numeric / nullif(sum(coalesce(atr.clicks, 0))::numeric, 0)) * 100
...
where coalesce(lower(atr.campaign_name), '') not like 'dev%'
  and coalesce(lower(atr.campaign_name), '') not like '%placeholder%'
group by atr.workspace_id, atr.metric_date, atr.campaign_name;

grant select on public.v_unified_ads_performance_daily to authenticated;

create or replace view public.v_unified_ads_performance_summary
with (security_invoker = true)
as
select
  d.workspace_id,
...
create or replace view public.v_unified_sales_performance_daily
with (security_invoker = true)
as
select
  rs.workspace_id,
  ...
  sum(coalesce(rs.total_payment_usd, 0))::numeric as total_payment_usd,
  sum(coalesce(rs.total_payment_uah, 0))::numeric as total_payment_uah,
  'raw_sales'::text as source_layer
from public.raw_sales rs
where coalesce(lower(rs.campaign_name), '') not like 'dev%'
  and coalesce(lower(rs.campaign_name), '') not like '%placeholder%'
  and coalesce(lower(rs.customer_name), '') not like 'test%'
group by rs.workspace_id, rs.sale_date, rs.campaign_name;

grant select on public.v_unified_sales_performance_daily to authenticated;

create or replace view public.v_unified_sales_performance_summary
with (security_invoker = true)
as
select
  d.workspace_id,
...
create or replace view public.v_unified_funnel_stage_summary
with (security_invoker = true)
as
select
  fe.workspace_id,
  ...
  count(distinct fe.contact_id)::bigint as unique_contacts,
  min(fe.event_date) as first_date,
  max(fe.event_date) as last_date
from public.v_funnel_events fe
where coalesce(lower(fe.campaign_name), '') not like 'dev%'
  and coalesce(lower(fe.campaign_name), '') not like '%placeholder%'
group by fe.workspace_id, fe.event_type;

grant select on public.v_unified_funnel_stage_summary to authenticated;

create or replace view public.v_unified_funnel_conversion_summary
with (security_invoker = true)
as
