...
create or replace function public.complete_tiktok_ads_oauth_connection(
  ...
  p_provider_account_email text default null::text,
  p_advertiser_id text default null::text,
  p_advertiser_name text default null::text,
  p_scopes jsonb default '[]'::jsonb,
  p_access_token text default null::text,
  p_refresh_token text default null::text,
  p_token_expires_at timestamp with time zone default null::timestamp with time zone,
  p_refresh_token_expires_at timestamp with time zone default null::timestamp with time zone,
...
begin
  ...
  v_vault_secret_name := 'tiktok_ads_oauth_'
    || replace(v_state.workspace_id::text, '-', '_')
    || '_'
    || substr(md5(p_state_token), 1, 12);
  ...
  v_vault_payload := jsonb_build_object(
    ...
    'refresh_token_expires_at', case
      when v_has_refresh_token then p_refresh_token_expires_at
