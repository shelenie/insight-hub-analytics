...
export default function Campaigns() {
  ...
  return <DashboardLayout title={t("campaignsTitle")} subtitle={t("campaignsSubtitle")}><div className="space-y-4"><FilterBar extra={<Input value={queryText} onChange={(e) => setQueryText(e.target.value)} placeholder={t("searchPlaceholder")} className="h-8 w-[240px] text-xs" />} freshness={{ source: "v_unified_ads_performance_daily", status: "fresh", lastSync: "live" }} />
    ...
    <SectionCard title="Підсумок реклами" description="Зведені метрики імпортованого трафіку" noPadding>
      {summaryRows.length === 0 ? <Msg t="Рекламні дані поки не знайдені. Перевірте імпорт трафіку або підключення джерел." /> : <Kpi rows={[{ label: "Витрати", value: fmtCurrency(totals.spend) }, { label: "Кліки", value: fmtNum(totals.clicks) }, { label: "Ліди", value: fmtNum(totals.leads) }, { label: "Охоплення", value: fmtNum(totals.reach) }, { label: "CPC", value: cpc == null ? "—" : fmtCurrency(cpc) }, { label: "CPL", value: cpl == null ? "—" : fmtCurrency(cpl) }]} />}
    </SectionCard>
    ...
    <details className="rounded border">
      ...
      <SectionCard title="Стан звʼязків" description="Підключені рекламні акаунти (опційно)" noPadding><Simple rows={query.data?.bindings.rows ?? []} columns={[{ key: "platform", label: "Платформа" }, { key: "ad_account_name", label: "Рекламний акаунт" }, { key: "mapping_status", label: "Статус мапінгу" }, { key: "binding_status", label: "Статус звʼязку" }, { key: "updated_at", label: "Оновлено" }]} empty="Додаткові дані про звʼязки недоступні." /></SectionCard>
    </details>
    <details className="rounded border">
      <summary className="cursor-pointer px-4 py-3 text-sm font-medium">Додатково: аномалії</summary>
      <SectionCard title="Аномалії" description="Потенційні аномалії кампаній (опційно)" noPadding><Simple rows={query.data?.anomalies.rows ?? []} columns={[{ key: "severity", label: "Рівень" }, { key: "title", label: "Назва" }, { key: "reason", label: "Причина" }, { key: "created_at", label: "Створено" }]} empty="Додаткові дані про аномалії недоступні." /></SectionCard>
    </details>
  </div></DashboardLayout>;
}
