...
export default function Campaigns() {
  ...
  return <DashboardLayout title={t("campaignsTitle")} subtitle={t("campaignsSubtitle")}><div className="space-y-4"><FilterBar extra={<Input value={queryText} onChange={(e) => setQueryText(e.target.value)} placeholder={t("searchPlaceholder")} className="h-8 w-[240px] text-xs" />} freshness={{ source: "v_ads_performance_daily", status: "fresh", lastSync: "live" }} />
    {!session ? <Msg t="Sign in to view campaigns production data." /> : query.isLoading ? <Msg t="Завантаження campaigns production data…" /> : null}
    {connectorStatus === "no_active_connections" ? <Msg t="Connect a real ads account to activate ads data." /> : null}
    <SectionCard title="Метрики кампаній" description="Ефективність рекламних кампаній" noPadding>
      {(query.data?.daily.unavailableReason) ? <Msg t="No campaign data is available yet." /> : <Table><TableHeader><TableRow><TableHead>Платформа</TableHead><TableHead>Кампанія</TableHead><TableHead className="text-right">Витрати</TableHead><TableHead className="text-right">Кліки</TableHead><TableHead className="text-right">Покази / охоплення</TableHead><TableHead className="text-right">CTR</TableHead><TableHead className="text-right">CPC</TableHead><TableHead className="text-right">CPM</TableHead></TableRow></TableHeader><TableBody>{rows.slice(0, 100).map((r, i) => <TableRow key={i}><TableCell>{String(r.platform ?? "—")}</TableCell><TableCell>{String(r.campaign_name ?? r.campaign_id ?? "—")}</TableCell><TableCell className="text-right num">{val(r.spend, true)}</TableCell><TableCell className="text-right num">{val(r.clicks)}</TableCell><TableCell className="text-right num">{val(r.impressions ?? r.reach)}</TableCell><TableCell className="text-right num">{val(r.ctr)}</TableCell><TableCell className="text-right num">{val(r.cpc, true)}</TableCell><TableCell className="text-right num">{val(r.cpm, true)}</TableCell></TableRow>)}</TableBody></Table>}
    </SectionCard>
    <SectionCard title="Стан звʼязків" description="Підключені рекламні акаунти" noPadding><Simple rows={query.data?.bindings.rows ?? []} columns={["platform", "ad_account_name", "mapping_status", "binding_status", "updated_at"]} empty="Binding status unavailable." /></SectionCard>
    <SectionCard title="Аномалії" description="Потенційні аномалії кампаній" noPadding><Simple rows={query.data?.anomalies.rows ?? []} columns={["severity", "title", "reason", "created_at"]} empty="No anomaly candidates." /></SectionCard>
  </div></DashboardLayout>;
}
