...
export default function Alerts() {
  ...
  return <DashboardLayout title="Telegram / Alerts" subtitle="Inspect Telegram routing, pending work queues, operational alerts, and HITL health.">
    <p className="mb-4 text-xs text-muted-foreground">{statusMessage}</p>{roleLoading ? <p className="mb-4 text-xs text-muted-foreground">Loading workspace role permissions…</p> : null}{!roleLoading && !canManage ? <p className="mb-4 text-xs text-muted-foreground">You do not have permission to manage Telegram alerts.</p> : null}{!roleLoading && roleError ? <p className="mb-4 text-xs text-muted-foreground">Workspace role unavailable. Write actions are disabled for safety.</p> : null}
    {!session ? <SectionCard title="Telegram / Alerts" description="Authentication required"><p className="text-sm text-muted-foreground">You are signed out. Sign in to access Telegram and operational alerts data.</p></SectionCard>
      : query.isLoading ? <SectionCard title="Telegram / Alerts" description="Loading data"><p className="text-sm text-muted-foreground">Loading alerts workspace…</p></SectionCard>
        : query.error ? <SectionCard title="Telegram / Alerts" description="Error state"><p className="text-sm text-destructive">Could not load alerts workspace: {query.error.message}</p></SectionCard>
          : <Tabs defaultValue="overview" className="space-y-4"><TabsList className="w-full justify-start overflow-x-auto"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="chats">Telegram Chats</TabsTrigger><TabsTrigger value="routes">Notification Routes</TabsTrigger><TabsTrigger value="outbox">Pending Outbox</TabsTrigger><TabsTrigger value="action-requests">Pending Action Requests</TabsTrigger><TabsTrigger value="alerts">Operational Alerts</TabsTrigger><TabsTrigger value="health">Health</TabsTrigger></TabsList>
            ...
            <TabsContent value="chats"><SectionCard title="Telegram Chats" description="Connected chats"><KnownColumnsTable rows={query.data?.telegramChats.rows ?? []} columns={["chat_title", "chat_name", "title", "name", "chat_type", "type", "chat_id", "status", "created_at", "updated_at"]} emptyText="No Telegram chats found." /><UnavailableHint data={query.data?.telegramChats} /></SectionCard></TabsContent>
            <TabsContent value="routes"><SectionCard title="Notification Routes" description="Notification destinations"><KnownColumnsTable rows={query.data?.telegramRoutes.rows ?? []} columns={["route_name", "route_type", "route", "destination_chat", "destination_chat_id", "chat_id", "event_type", "enabled", "status"]} emptyText="No notification routes found." /><UnavailableHint data={query.data?.telegramRoutes} /></SectionCard></TabsContent>
            <TabsContent value="outbox"><SectionCard title="Pending Outbox" description="Messages waiting to send"><KnownColumnsTable rows={query.data?.outboxPending.rows ?? []} columns={["message_type", "status", "destination_chat", "destination_chat_id", "chat_id", "created_at", "error", "error_message"]} emptyText="No pending outbox messages found." /><div className="mt-4 rounded-md border border-dashed border-border/70 bg-muted/30 p-3"><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" disabled={!session || !canManage || !getOutboxMessageId(firstOutbox) || pendingAction === "retry-outbox"} onClick={() => { const outboxId = getOutboxMessageId(firstOutbox); if (!outboxId) return setStatusMessage("Missing outbox_message_id on the selected row."); void runAction("retry-outbox", () => supabase.functions.invoke("telegram-outbox-retry", { body: { workspace_id: WORKSPACE_ID, outbox_message_id: outboxId } })); }}>{pendingAction === "retry-outbox" ? "Retrying…" : "Retry message"}</Button></div></div><UnavailableHint data={query.data?.outboxPending} /></SectionCard></TabsContent>
            <TabsContent value="action-requests"><SectionCard title="Pending Action Requests" description="Requests pending review"><KnownColumnsTable rows={query.data?.actionRequestsPending.rows ?? []} columns={["action_type", "status", "requested_by", "requested_by_user", "created_at", "related_object", "related_object_id", "object_type"]} emptyText="No pending action requests found." /><div className="mt-4 rounded-md border border-dashed border-border/70 bg-muted/30 p-3"><div className="flex flex-wrap gap-2"><Button type="button" disabled={!session || !firstActionRequest || pendingAction === "open-action-request"} onClick={() => {
...
function OptionalViewCard({ title, viewName, data }: { title: string; viewName: string; data: OptionalViewData | undefined }) { return <SectionCard title={title} description="Details">{data?.unavailableReason ? <p className="text-sm text-muted-foreground">Unavailable: {data.unavailableReason}</p> : <GenericTable rows={data?.rows ?? []} emptyText="No records found." />}</SectionCard>; }
