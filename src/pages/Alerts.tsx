...
export default function Alerts() {
  ...
  const refreshAlerts = async () => {
    await query.refetch();
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ["telegram-alerts-workspace", WORKSPACE_ID] }),
      queryClient.invalidateQueries({ queryKey: ["v_telegram_outbox_pending", WORKSPACE_ID] }),
      queryClient.invalidateQueries({ queryKey: ["v_telegram_action_requests_pending", WORKSPACE_ID] }),
      queryClient.invalidateQueries({ queryKey: ["v_operational_alerts_recent", WORKSPACE_ID] }),
      queryClient.invalidateQueries({ queryKey: ["v_operational_alerts_health", WORKSPACE_ID] }),
      queryClient.invalidateQueries({ queryKey: ["v_telegram_hitl_health", WORKSPACE_ID] }),
      queryClient.invalidateQueries({ queryKey: ["v_telegram_hitl_production_health", WORKSPACE_ID] }),
    ]);
  };
  ...
  const runAction = async (key: string, fn: () => Promise<{ data: unknown; error: { message: string } | null }>) => {
    ...
    toast({ title: "Success", description: "Telegram/alert action completed." });
    await refreshAlerts();
  };
  ...
  return (
    <DashboardLayout title="Telegram / Alerts" subtitle="Inspect Telegram routing, pending work queues, operational alerts, and HITL health.">
      <p className="mb-4 text-xs text-muted-foreground">{statusMessage}</p>
      {!session ? (
        ...
        <Tabs defaultValue="overview" className="space-y-4">
          <TabsList className="w-full justify-start overflow-x-auto">
            <TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="chats">Telegram Chats</TabsTrigger><TabsTrigger value="routes">Notification Routes</TabsTrigger><TabsTrigger value="outbox">Pending Outbox</TabsTrigger><TabsTrigger value="action-requests">Pending Action Requests</TabsTrigger><TabsTrigger value="alerts">Operational Alerts</TabsTrigger><TabsTrigger value="health">Health</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="chats"><SectionCard title="Telegram Chats" description="Source: v_telegram_chats"><KnownColumnsTable rows={query.data?.telegramChats.rows ?? []} columns={["chat_title", "chat_name", "title", "name", "chat_type", "type", "chat_id", "status", "created_at", "updated_at"]} emptyText="No Telegram chats found." /><UnavailableHint data={query.data?.telegramChats} /></SectionCard></TabsContent>
          <TabsContent value="routes"><SectionCard title="Notification Routes" description="Source: v_telegram_notification_routes"><KnownColumnsTable rows={query.data?.telegramRoutes.rows ?? []} columns={["route_name", "route_type", "route", "destination_chat", "destination_chat_id", "chat_id", "event_type", "enabled", "status"]} emptyText="No notification routes found." /><UnavailableHint data={query.data?.telegramRoutes} /></SectionCard></TabsContent>

          <TabsContent value="outbox"><SectionCard title="Pending Outbox" description="Source: v_telegram_outbox_pending"><KnownColumnsTable rows={query.data?.outboxPending.rows ?? []} columns={["message_type", "status", "destination_chat", "destination_chat_id", "chat_id", "created_at", "error", "error_message"]} emptyText="No pending outbox messages found." /><div className="mt-4 rounded-md border border-dashed border-border/70 bg-muted/30 p-3"><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" disabled={!session || !getOutboxMessageId(firstOutbox) || pendingAction === "retry-outbox"} onClick={() => {
            const outboxId = getOutboxMessageId(firstOutbox);
            if (!outboxId) return setStatusMessage("Missing outbox_message_id on the selected row.");
            void runAction("retry-outbox", () => supabase.functions.invoke("telegram-outbox-retry", { body: { workspace_id: WORKSPACE_ID, outbox_message_id: outboxId } }));
          }}>{pendingAction === "retry-outbox" ? "Retrying…" : "Retry message"}</Button></div></div><UnavailableHint data={query.data?.outboxPending} /></SectionCard></TabsContent>

          <TabsContent value="action-requests"><SectionCard title="Pending Action Requests" description="Source: v_telegram_action_requests_pending"><KnownColumnsTable rows={query.data?.actionRequestsPending.rows ?? []} columns={["action_type", "status", "requested_by", "requested_by_user", "created_at", "related_object", "related_object_id", "object_type"]} emptyText="No pending action requests found." /><div className="mt-4 rounded-md border border-dashed border-border/70 bg-muted/30 p-3"><div className="flex flex-wrap gap-2"><Button type="button" disabled={!session || !getActionRequestId(firstActionRequest) || pendingAction === "open-action-request"} onClick={() => {
            const actionRequestId = getActionRequestId(firstActionRequest);
            if (!actionRequestId) return setStatusMessage("Missing action_request_id on the selected row.");
            void runAction("open-action-request", () => supabase.functions.invoke("telegram-action-request-open", { body: { workspace_id: WORKSPACE_ID, action_request_id: actionRequestId } }));
          }}>{pendingAction === "open-action-request" ? "Opening…" : "Open action request"}</Button></div></div><UnavailableHint data={query.data?.actionRequestsPending} /></SectionCard></TabsContent>

          <TabsContent value="alerts"><SectionCard title="Operational Alerts" description="Source: v_operational_alerts_recent"><KnownColumnsTable rows={query.data?.operationalAlertsRecent.rows ?? []} columns={["alert_type", "severity", "status", "title", "message", "created_at", "resolved_at"]} emptyText="No open alerts." /><div className="mt-4 rounded-md border border-dashed border-border/70 bg-muted/30 p-3"><div className="flex flex-wrap gap-2"><Button type="button" disabled={!session || !getAlertId(firstAlert) || pendingAction === "resolve-alert"} onClick={() => {
            const alertId = getAlertId(firstAlert);
            if (!alertId) return setStatusMessage("Missing alert_id on the selected row.");
            void runAction("resolve-alert", () => supabase.functions.invoke("operational-alert-resolve", { body: { workspace_id: WORKSPACE_ID, alert_id: alertId } }));
          }}>{pendingAction === "resolve-alert" ? "Resolving…" : "Resolve alert"}</Button></div></div><UnavailableHint data={query.data?.operationalAlertsRecent} /></SectionCard></TabsContent>
...
const getOutboxMessageId = (row: Row | undefined) => String(row?.outbox_message_id ?? row?.message_id ?? row?.id ?? "");
const getActionRequestId = (row: Row | undefined) => String(row?.action_request_id ?? row?.id ?? row?.request_id ?? "");
const getAlertId = (row: Row | undefined) => String(row?.alert_id ?? row?.id ?? "");

function UnavailableHint({ data }: { data: OptionalViewData | undefined }) { if (!data?.unavailableReason) return null; return <p className="mt-2 text-xs text-muted-foreground">This view is partially unavailable: {data.unavailableReason}</p>; }
