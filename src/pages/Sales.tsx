    ...
    <div className="sales-page space-y-4 overflow-x-hidden">
      ...
      <SectionCard title={lang === "uk" ? "Підсумок продажів" : "Sales summary"} description={lang === "uk" ? "Ключові фінансові показники за вибраний період" : "Key financial metrics for the selected period"}>
        {hasSalesDataError ? <Msg t={t("salesLoadError")} /> : showSummaryEmpty ? <Msg t={t("salesEmpty")} /> : <>
          <Kpi rows={[
            { label: lang === "uk" ? "Продажі" : "Sales", value: fmtNum(totals.sales_count), description: lang === "uk" ? "Кількість продажів у вибраному періоді." : "Number of sales in the selected period.", delta: buildDelta(totals.sales_count, comparisonTotals.sales_count, fmtNum, compareDisplay, showDeltas) },
  ...
  return <div className="sales-table-scroll sales-buyers-table overflow-x-auto"><Table className="table-fixed" style={{ width: tableWidth > 900 ? tableWidth : "100%" }}><colgroup>{columns.map((column) => <col key={column.key} style={{ width: widths[column.key] ?? column.width }} />)}</colgroup><TableHeader><TableRow>{columns.map((column) => <TableHead key={column.key} className={`${STICKY_TABLE_HEAD_CLASS} px-1.5 text-[10.5px] uppercase tracking-wide ${column.align === "right" ? "text-right" : "text-left"}`}><span className="block truncate" title={column.tooltip ?? column.label}>{column.label}</span><span role="separator" aria-orientation="vertical" aria-label={locale === "uk" ? `Змінити ширину: ${column.label}` : `Resize column: ${column.label}`} title={locale === "uk" ? "Потягніть, щоб змінити ширину колонки" : "Drag to resize column"} onMouseDown={(event) => startBuyerColumnResize(event, column, widths, setWidths)} className="absolute right-0 top-1/2 h-6 w-[4px] -translate-y-1/2 cursor-col-resize touch-none rounded-full bg-border transition hover:bg-primary" /></TableHead>)}</TableRow></TableHeader><TableBody>
    {visibleRows.map((r, i) => {
      ...
      return <TableRow key={rowKey} onClick={() => setSelectedRowKey((current) => current === rowKey ? null : rowKey)} className={`cursor-pointer ${isSelected ? "bg-primary/10 hover:bg-primary/15" : "hover:bg-muted/50"}`}>
        ...
        <TableCell className="px-1.5 text-sm" title={formatPaymentType(r, locale)}><div className="flex flex-col items-start gap-1"><span className="max-w-full truncate whitespace-nowrap">{formatPaymentType(r, locale)}</span>{!hasPaidAmount ? <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-700" title={locale === "uk" ? "У джерелі немає суми оплати для цього запису" : "The source data has no payment amount for this record"}>{locale === "uk" ? "без суми" : "no amount"}</span> : null}</div></TableCell>
