...
export const translations = {
  ...
  conversionsBetweenStagesDesc: { uk: "Рахується за кількістю подій у вибраному періоді. Значення можуть бути понад 100%, якщо наступний етап має більше подій або частина попередніх подій була поза періодом.", en: "Calculated from event counts in the selected period. Values can exceed 100% when the next stage has more events or previous-stage events happened outside the selected period." },
  ...
  conversionsAbove100Hint: { uk: "Понад 100% — перевірте шлях або період", en: "Above 100% — check path or period" },
