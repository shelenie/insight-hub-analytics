...
describe("Step 3 Data Bindings frontend production flow", () => {
  ...
  it("implements two-phase selected source rebind and partial-success warning", () => {
    expect(bindings).toContain('supabase.functions.invoke("binding-create-or-update"');
  ...
  it("archives exact selected source and ad-account rows through a dialog", () => {
    ...
    expect(translations).toContain("Only this selected binding will be archived");
  });
  ...
  it("initializes full ad form state and creates ad bindings as not primary by default", () => {
    ...
    expect(mutations).toContain("p_is_primary: primaryIntentValue(input.primaryIntent)");
  });
  ...
  it("sends exact selected ad replacement IDs without searching unrelated scopes", () => {
    ...
    expect(bindings).toContain("replaceBindingId: sameScope ? null : normalAdForm.binding_id || null");
    expect(bindings).not.toContain("find((binding) => asText(binding.ad_account_id)");
  });
