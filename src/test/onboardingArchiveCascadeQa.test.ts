...
const migration = readFileSync(
  "supabase/migrations/20260713_onboarding_archive_cascade_rpcs.sql",
  "utf8",
);
const clientWrapper = readFileSync(
  "supabase/functions/onboarding-client-upsert/index.ts",
  "utf8",
);
const projectWrapper = readFileSync(
  "supabase/functions/onboarding-project-upsert/index.ts",
  "utf8",
);
const funnelWrapper = readFileSync(
...
describe("onboarding archive cascade production QA fix", () => {
  it("adds transactional RPCs for client, project, and funnel archive cascades", () => {
    ...
    expect(migration).toContain(
      "public.require_source_manager(p_workspace_id)",
    );
    expect(migration).not.toMatch(
      /delete\s+from\s+public\.(clients|projects|funnels|ad_account_bindings|source_entity_bindings)/i,
    );
  });

  it("client archive cascades to projects, funnels, and active bindings idempotently", () => {
    expect(migration).toContain(
      "where workspace_id = p_workspace_id and client_id = p_client_id and coalesce(status::text, 'active') not in",
    );
    expect(migration).toContain(
  ...
  it("project and funnel archive run only for active-to-inactive transitions", () => {
    ...
    expect(projectWrapper).toContain(
      'userClient["rpc"]("archive_onboarding_project_cascade"',
    );
    expect(funnelWrapper).toContain(
      'userClient["rpc"]("archive_onboarding_funnel_cascade"',
    );
    expect(projectWrapper.indexOf("if (archiveTransition)")).toBeLessThan(
      projectWrapper.indexOf(
        "if (!transition.existingInactive || reactivationTransition)",
      ),
    );
    expect(funnelWrapper.indexOf("if (archiveTransition)")).toBeLessThan(
      funnelWrapper.indexOf(
        "const projectCheck = !transition.existingInactive || reactivationTransition",
      ),
    );
    expect(migration).toContain(
      "where workspace_id = p_workspace_id and project_id = p_project_id and coalesce(binding_status::text, 'active') = 'active'",
    );
    expect(migration).toContain(
      "where workspace_id = p_workspace_id and funnel_id = p_funnel_id and coalesce(binding_status::text, 'active') = 'active'",
    );
  });
  ...
  it("inactive-to-inactive edits skip parent guards while reactivation and active edits keep them", () => {
    ...
    expect(projectWrapper).toContain(
      "const clientCheck = await requireActiveClient",
    );
    expect(funnelWrapper).toContain(
      "const projectCheck = !transition.existingInactive || reactivationTransition",
    );
  ...
  it("migration signatures and grants remain restricted", () => {
    for (const name of ["client", "project", "funnel"]) {
      expect(migration).toContain(
        `archive_onboarding_${name}_cascade(uuid, uuid, text, jsonb)`,
      );
      expect(migration).toContain(
        `revoke all on function public.archive_onboarding_${name}_cascade(uuid, uuid, text, jsonb) from public, anon`,
      );
      expect(migration).toContain(
        `grant execute on function public.archive_onboarding_${name}_cascade(uuid, uuid, text, jsonb) to authenticated, service_role`,
      );
    }
