...
const diagnosticsAiContextMigrationSource = readFileSync(
...
describe("AI ads context backend contract", () => {
  it("calls build_ai_ads_context without unsupported p_context_scope", () => {
    const buildAdsContextBody =
      edgeFunctionSource.match(
        /async function buildAdsContext[\s\S]*?async function buildImportContext/,
      )?.[0] ?? "";

    expect(buildAdsContextBody).toContain('"build_ai_ads_context"');
    expect(buildAdsContextBody).toContain("payload");
  ...
  it("passes adaptive response requirements into the OpenAI user prompt", () => {
    ...
    expect(userPromptBody).toContain(
      "do not invent revenue, ROAS, LTV, CAC payback",
    );
    expect(userPromptBody).toContain(
      "do not expose raw backend field names in the main answer",
    );
  });
  ...
  it("uses the correct date column for facts and unified ads sources", () => {
    const factsCountBlock =
      migrationSource.match(
        /from public\.facts_ads_daily[\s\S]*?using p_workspace_id, p_date_from, p_date_to, p_platform;/,
      )?.[0] ?? "";
    const factsBranchBlock =
      migrationSource.match(/if v_fact_rows > 0 then[\s\S]*?else/)?.[0] ?? "";
    const unifiedCountBlock =
      migrationSource.match(
        /from public\.v_unified_ads_performance_daily[\s\S]*?using p_workspace_id, p_date_from, p_date_to;/,
      )?.[0] ?? "";
    const topCampaignsBlock =
      migrationSource.match(
        /if v_daily_source is not null and v_daily_date_column is not null then[\s\S]*?end if;/,
      )?.[0] ?? "";
  ...
  it("adds a safe ads pipeline diagnostics RPC without exposing token or secret fields", () => {
    expect(diagnosticsMigrationSource).toContain(
    ...
    expect(diagnosticsMigrationSource).toContain(
      "active_platform_connections_by_platform",
    );
    ...
    expect(diagnosticsMigrationSource).toContain(
      "latest_ad_sync_run_logs_by_platform",
    );
  ...
  it("detects the ad_traffic_raw date column dynamically for production day schema", () => {
    const trafficRawBlock =
      diagnosticsMigrationSource.match(
        /if to_regclass\('public\.ad_traffic_raw'\) is not null then[\s\S]*?v_has_raw := v_has_raw/,
      )?.[0] ?? "";
    const compactTrafficRawBlock = trafficRawBlock.replace(/\s+/g, " ");

    expect(trafficRawBlock).toContain("information_schema.columns");
    expect(compactTrafficRawBlock).toContain(
      "from (values ('metric_date', 1), ('day', 2), ('insight_date', 3))",
    );
  ...
  it("keeps latest failed sync diagnostics JSON fields paired correctly", () => {
    const latestFailedEnd = diagnosticsMigrationSource.indexOf(
      "into v_latest_failed using p_workspace_id;",
    );
    const latestFailedBlock =
      latestFailedEnd === -1
        ? ""
        : diagnosticsMigrationSource.slice(
            diagnosticsMigrationSource.lastIndexOf(
              "with runs as",
              latestFailedEnd,
            ),
            latestFailedEnd,
          );
    const compactLatestFailedBlock = latestFailedBlock.replace(/\s+/g, " ");

    expect(compactLatestFailedBlock).toContain(
      "select platform, date_from, date_to, error_message",
    );
    expect(compactLatestFailedBlock).toContain(
      "jsonb_build_object('platform', platform::text, 'date_from', date_from, 'date_to', date_to, 'error_message'",
    );
    expect(compactLatestFailedBlock).toContain(
      "left(regexp_replace(coalesce(error_message, '')",
    );
  });
  ...
  it("extends build_ai_ads_context with diagnostics while preserving its signature", () => {
    expect(diagnosticsAiContextMigrationSource).toContain(
      "create or replace function public.build_ai_ads_context(\n  p_workspace_id uuid,\n  p_date_from date default null,\n  p_date_to date default null,\n  p_platform text default null\n)",
    );
    expect(diagnosticsAiContextMigrationSource).not.toContain(
      "p_context_scope",
    );
    expect(diagnosticsAiContextMigrationSource).toContain(
      "public.build_ads_pipeline_diagnostics(p_workspace_id, p_date_from, p_date_to)",
    );
    expect(diagnosticsAiContextMigrationSource).toContain(
