...
describe("Bindings confirmed UI defects", () => {
  ...
  it("uses fixed-pixel Source table columns with a compact primary column", () => {
    ...
    expect(columns.map((col) => col.getAttribute("style"))).toEqual([
      "width: 220px;",
      "width: 120px;",
      "width: 115px;",
      "width: 110px;",
  ...
  it("vertically centers body cells in both business tables", () => {
    const { container: sourceContainer } = renderWithI18n(
      <SourceBindingsBusinessTable rows={[activeRow]} canManage={true} roleLoading={false} onEdit={vi.fn()} onArchive={vi.fn()} onRestore={vi.fn()} />,
    );
    expect(Array.from(sourceContainer.querySelectorAll("tbody td")).every((cell) => cell.classList.contains("align-middle"))).toBe(true);
    expect(sourceContainer.querySelector("tbody tr")?.className).not.toContain("align-top");

    const { container: adContainer } = renderWithI18n(
      <AdAccountsBusinessTable rows={[{ ...activeRow, external_account_id: "act_1", external_account_name: "Account", platform: "meta" }]} canManage={true} roleLoading={false} onEdit={vi.fn()} onArchive={vi.fn()} onRestore={vi.fn()} />,
    );
    expect(Array.from(adContainer.querySelectorAll("tbody td")).every((cell) => cell.classList.contains("align-middle"))).toBe(true);
    expect(adContainer.querySelector("tbody tr")?.className).not.toContain("align-top");
  });
  ...
  it("renders active Source actions as compact full-width action buttons", () => {
    ...
    expect(actionContainer).toHaveClass("flex", "w-full", "flex-col", "items-stretch", "justify-center", "gap-2");
    ...
    for (const button of buttons) expect(button).toHaveClass("w-full", "max-w-full", "justify-center", "whitespace-nowrap", "h-8", "px-2");
  });
  ...
  it("uses the same action container for archived Restore, permission loading, and Read-only", () => {
    ...
    expect(actionContainer).toHaveClass("flex", "w-full", "flex-col", "items-stretch", "justify-center");
    expect(screen.getByRole("button", { name: /відновити/i })).toHaveClass("w-full", "max-w-full", "justify-center", "whitespace-nowrap", "h-8", "px-2");
