...
import {
  cleanAssistantTextForModelContext,
...
describe("AI Assistant answer parsing", () => {
  it("strips leading Ukrainian context labels", () => {
    expect(
      stripLeadingContextLabel("Контекст: Стан рекламних підключень\n\nBody"),
    ).toBe("Body");
    expect(
      stripLeadingContextLabel("Контекст: Ефективність реклами\nBody"),
    ).toBe("Body");
  });
  ...
  it("serializes whole-answer copy without raw client copy marker lines", () => {
    const serialized = serializeAnswerForWholeCopy(
      "Intro\n\n[CLIENT_COPY_START]\nClient text only\n[CLIENT_COPY_END]\n\n## Внутрішньо: що перевірити\n- Access",
    );
  ...
  it("cleans model context and previews without raw client markers or leading context labels", () => {
    const raw =
      "Контекст: Стан рекламних підключень\n\nIntro\n[CLIENT_COPY_START]\nClient text\n[CLIENT_COPY_END]\nLater Контекст: keep this";

    expect(cleanAssistantTextForModelContext(raw)).toBe(
      "Intro\nClient text\nLater Контекст: keep this",
    );
    expect(cleanAssistantTextForPreview(raw)).toBe(
      "Intro Client text Later Контекст: keep this",
    );
  });

  it("cleans markdown markers from preview-only text", () => {
    expect(cleanAssistantTextForPreview("### Heading")).toBe("Heading");
    expect(cleanAssistantTextForPreview("**CPL**")).toBe("CPL");
    expect(cleanAssistantTextForPreview("*Cost Per Lead*")).toBe(
      "Cost Per Lead",
    );
    expect(
      cleanAssistantTextForPreview(
    ...
    ).toBe("CPL — це Cost Per Lead Client");
  });
});
